...
# 写入csv文件中
def WriteToCSV(csvName, text, temp, zp):
	if temp % 100 == 1 and zp == 0:
...
while urlTemp != None:
	...
	for zp in range(0, len(diseaseName),1):
		...
		if temp <= 100:
			WriteToCSV('ZhengZhuang_001-100.csv', info ,temp,zp)
		elif temp <= 200 and temp > 100:
			WriteToCSV('ZhengZhuang_101-200.csv', info ,temp,zp)
		elif temp <= 300 and temp > 200:
			WriteToCSV('ZhengZhuang_201-300.csv', info ,temp,zp)
		elif temp <= 400 and temp > 300:
			WriteToCSV('ZhengZhuang_301-400.csv', info ,temp,zp)
		elif temp <= 500 and temp > 400:
			WriteToCSV('ZhengZhuang_401-500.csv', info ,temp,zp)
		elif temp <= 600 and temp > 500:
			WriteToCSV('ZhengZhuang_501-600.csv', info ,temp,zp)
		else:
			 WriteToCSV('ZhengZhuang_601-700.csv', info ,temp,zp)
	temp = temp + 1
